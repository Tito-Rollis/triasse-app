export const DATA = [
    {
        why: [
            {
                title: 'Jaminan harga terbaik',
                text: 'Harga kompetitif di Laboratorium terbesar dan terpecaya di Indonesia seperti Prodia, Kimia Farma, Parahita, GS Lab & Biotest.',
            },
            {
                title: 'Layanan homoe service',
                text: 'Pemeriksaan bisa dilakukan di rumah sendiri dengan layanan Home Service, petugas datang untuk melakukan pengecekan dan tes.',
            },
            {
                title: 'Cicilan 4x dengan bunga 0%',
                text: 'Pembayaran paket pemeriksaan lebih ringan karena bisa dicicil sebanyak empat kali, dan tanpa bunga tambahan.',
            },
            {
                title: 'Pesan dan hasil online',
                text: 'Pesan dan hasil pemeriksaan bisa dilakukan secara online, proses lebih cepat tanpa perlu antri di lab.',
            },
        ],
    },
    {
        lorem: [
            {
                title: 'Paket Medical Check-up Basic',
                text: 'Harga mulai dari',
                price: 'Rp416.000',
                discount: 'Rp364.001',
            },
            {
                title: 'Paket Medical Check-up Silver',
                text: 'Harga mulai dari',
                price: 'Rp600.000',
                discount: 'Rp573.000',
            },
            {
                title: 'Paket Medical Check-up Gold',
                text: 'Harga mulai dari',
                price: 'Rp1.288.000',
                discount: 'Rp1.127.000',
            },
            {
                title: 'Paket Skrining Diabetes Melitus',
                text: 'Harga mulai dari',
                price: 'Rp261.625',
                discount: 'Rp249.375',
            },
            {
                title: 'Paket Risiko Penyakit Jantung',
                text: 'Harga mulai dari',
                price: 'Rp1.162.000',
                discount: 'Rp1.016.750',
            },
            {
                title: 'Paket TORCH',
                text: 'Harga mulai dari',
                price: 'Rp2.480.000',
                discount: 'Rp1.417.000',
            },
        ],
    },
    {
        mitra: [
            { src: '/img/img-mitra-1.png' },
            { src: '/img/img-mitra-2.png' },
            { src: '/img/img-mitra-3.png' },
            { src: '/img/img-mitra-4.png' },
            { src: '/img/img-mitra-5.png' },
        ],
    },
    {
        lorem_2: [
            { title: 'Rapidtest Covid-19 Sekarang, Bayar dengan EMPAT KALI' },
            { title: 'Apa Itu Meningitis? Semua Yang Anda Perlu Tahu' },
            { title: 'Telat Haid: Gejala, Penyebab, dan Cara Mengatasi' },
            { title: 'Ingin Sukses? Ketahui 6 Tips Malam Pertama Ini' },
        ],
    },
    {
        media: [
            { src: '/img/img-media-coverage-1.png' },
            { src: '/img/img-media-coverage-2.png' },
            { src: '/img/img-media-coverage-3.png' },
            { src: '/img/img-media-coverage-4.png' },
            { src: '/img/img-media-coverage-5.png' },
            { src: '/img/img-media-coverage-6.png' },
            { src: '/img/img-media-coverage-7.png' },
            { src: '/img/img-media-coverage-8.png' },
            { src: '/img/img-media-coverage-9.png' },
        ],
    },
    {
        text: [
            {
                title: 'Toko Furniture Online Tercepaya di Indonesia',
                text: 'Bramblefurniture.com adalah sebuah platform e-commerce milik Bramble Furniture yang menjual beragam pilihan furniture beserta dekorasi terlengkap yang dapat di-custom dari material kayu mahoni berkualitas. Ada lebih dari 500 produk yang bisa dibeli dengan bermacam pilihan finishing, dari warna hingga motif. jenis custom dan finishing yang dipilih  akan menentukan hasil akhir furniture yang diinginkan untuk kesan yang lebih personal dan tetap mengedepankan kualitas dan ketahanan.',
            },
            {
                title: 'Belanja Mebel Custom Terlengkap di Bramble Furniture',
                text: 'Sekarang belanja furniture menjadi semakin mudah dengan hadirnya toko Bramble Furniture online. Dapatkan furniture idaman terbaik hanya lewat beberapa klik! Di Bramble Furniture, pembeli dapat melihat seluruh detail furniture yang ingin dibeli secara lengkap, dari dimensi, spesifikasi, deskripsi, hingga review para pembeli. Setelahnya, pembeli dapat langsung menentukan opsi custom dan finishing yang diinginkan, mulai dari warna hingga penambahan artwork. Sudah waktunya belanja furniture terbaik dengan cepat dan mudah hanya dari gadget Anda!',
            },
            {
                title: 'Situs Jual Beli Furniture Custom Terbaik di Indonesia',
                text: 'Di Indonesia, kemunculan online store bermula untuk memberikan sistem jual beli yang mudah kepada masyarakat. selain itu, online store tentu juga harus memprioritaskan keamanan dan kenyamanan untuk proses beli dan jual barang. Dari itu, Brumble Furniture memiliki komitmen untuk menyediakan metode pembayaran yang terpercaya dan nyaman bagi seluruh calon pembeli. Adapun metode yang bisa dipilih melalui transfer bank, ATM, Internet Banking, dan cicilan dengan kartu kredit, berlogo VISA dan MasterCard. Belanja furniture online jadi semakin asyik dengan pilihan pembayaran yang aman, nyaman, dan fleksibel.',
            },
            {
                title: 'Online Store dengan Produk Furniture Berkualitas Premium',
                text: 'Apapun gaya interior rumah Anda, dari minimalis, modern, mid-century, hingga skandinavia akan jadi lebih hidup dan bergaya dengan pilihan furniture, mebel, dan perlengkapan yang tepat. Brumble Furniture menyediakan beragam gaya furniture yang cocok untuk gaya rumah masa kini, seperti gaya American Style, Parisian, Coastal, Urban, dan sebagainya. Temukan furniture dan perlengkapan yang sesuai untuk rumah Anda berdasarkan kategori produk kami, seperti Meja Tamu, Meja Kerja, Meja Makan, Meja Konsul, Rak Buku, Lemari Dapur, Kursi Panjang Panjang, Kursi Malas, Kursi Makan, Bangku, Lemari Pakaian, Lemari Pajangan, Kabinet Kamar Mandi, Kabinet Wastafel, Cermin, Lampu Gantung, Lampu Lantai, Lampu Meja, dan masih banyak lagi. Furniture yang dipilih dapat di-custom dari warna dan pengerjaannya untuk menunjang interior rumah masa kini, seperti minimalis, mid-century, modern, hingga skandinavia. Tunggu apalagi, dekor rumah impian Anda dengan pilihan furniture yang beragam dan berkualitas sekarang juga!',
            },
            {
                title: 'Grtis Ongkir Furniture Seluruh Jabodetabek',
                text: 'Dapatkan promo gratis ongkir untuk pengiriman furniture di wilayah Jakarta dan sekitarnya! Beli furniture online dengan tenang tanpa harus pusing dengan biaya pengiriman hanya di Bramble Furniture Indonesia.',
            },
        ],
    },
];
